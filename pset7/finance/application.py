...
def history():
    ...
    try:
        stocks = db.execute("SELECT symbol, shares, time FROM transactions WHERE user_id = :user_id AND shares != 0" , user_id=session["user_id"])
        for i in range(len(stocks)):
            data = lookup(stocks[i]['symbol'])
            stocks[i]['price'] = data['price']
            stocks[i]['name'] = data['name']
...
def quote():
    ...
    if request.method == "POST":
        quote = lookup(request.form.get("symbol"))
        if quote == None:
            return apology("Please only type in the company symbol")
        else:
