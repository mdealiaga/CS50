...
###############START: EXTRA IMPLEMENTATION TO BUY AND SELL################
    if request.method == "POST":
        ...
        print('stocks and price above test')
        #DO USER BUYING
        req_quant_list = {}
        symbol_loop = []
        req_quant_loop []
        ...
        total_cost = 0
        #CREATE DICTIONARY OF REQUESTED QUANTIIES BUY> SYMBOL: REQ QUANT
        for i in range(len(stocks)):
            #ENSURES STRINGS ARE ZERO INSTEAD OF EMPTY
            if len(request.form.get("buy_share_quant_" + str(stocks[i]['symbol']))) == 0:
                req_quant_list[stocks[i]['symbol']] = 0
                symbol_loop.append(i, stocks[i]['symbol'])
                req_quant_loop.append(i,)
            else:
                req_quant_list[stocks[i]['symbol']] = request.form.get("buy_share_quant_" + str(stocks[i]['symbol']))
        ...
        if float(user_cash[0]['cash']) >= float(total_cost):
            for i in range(len(req_quant_list)):
                symbol_loop = req_quant_list[i]
                req_quant_loop = 
                db.execute("UPDATE users SET cash = cash - :total_cost WHERE id = :user_id", total_cost=total_cost, user_id=session["user_id"])
                db.execute("INSERT INTO transactions (user_id, symbol, shares, price, time) VALUES(:user_id, :symbol, :shares, :price, :datetime)", user_id=session["user_id"], symbol=symbol_loop, shares=req_quant_loop, price=total_cost, datetime = time.strftime('%Y-%m-%d %H:%M:%S'))
        ...
        print("test below")
        print(req_quant_list)
        print(len(req_quant_list))
        print("total_cost below")
        print(total_cost)
